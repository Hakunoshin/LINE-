...
#### 投稿末尾の導線（CTA）と自動A/Bテスト

各投稿の末尾には、**「DMへ誘導」**または**「コメントに一言を誘導」**のどちらかの一文が入ります。
どちらが効くかは、エンゲージメント（いいね＋返信＋リポスト＋引用）を指標に**自動でA/Bテスト（ε-greedy）**します。

- 各CTAが一定数（3件）たまるまでは 50/50 でランダムに出して探索
- 十分たまったら、平均エンゲージメントが高い方を確率 75% で採用しつつ、25% は引き続き探索
- 秘書からのLINE通知や `求人投稿` の返信に、その投稿で使った導線（DM誘導/コメント誘導）が表示されます

> 注意: Threads APIで取得できるのは閲覧数・いいね・返信・リポスト・引用までで、**DMの実数は取得できません**。そのためA/Bはエンゲージメント全体を代理指標として判定します。

#### パフォーマンス分析→改善ループ

投稿後、Threads Insights（閲覧数・いいね・返信・リポスト・引用）を毎日 `THREADS_INSIGHTS_TIME_JST`（デフォルト `23:30` JST）に取得してD1に蓄積します。
上位の投稿をClaudeが分析し、「どんな投稿が伸びるか」「どちらのCTAが効くか」の改善メモを更新。次回以降の投稿生成にそのメモを渡すことで、投稿が継続的に改善されていきます。
...
migrations/
  ...
  0005_cta_type.sql      threads_post_metricsにcta_type列を追加(CTAのA/B用)
```
