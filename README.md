...
X案 / Xテスト          … いま投稿案を作ってLINEに送る（承認待ち・動作確認にも）
Xキュー                … 承認待ち/投稿待ちの一覧
X設定                  … ネタ元・連携状況の確認
Xネタ元削除 3          … 登録したネタ元を削除
```
...
- 毎日 `X_GENERATE_TIME_JST`（既定 08:00 / 12:00 / 17:00 / 21:00 JST。カンマ区切りで変更可）に、登録済みネタ元から `X_DRAFTS_PER_RUN`（既定3件）を自動生成してLINEに送ります（ネタ元未登録なら送りません）。
